...
import { ADMIN_USERS_CHARGE_API } from "../../../../../core/api/urls/urls";
...
async function getUsers(page: number = 0, size: number = 5): Promise<TablePaginator<EditableUser>> {
    const users = await api.get<TablePaginator<EditableUserResponse>>(ADMIN_USERS_CHARGE_API, {params: {page, size}});
